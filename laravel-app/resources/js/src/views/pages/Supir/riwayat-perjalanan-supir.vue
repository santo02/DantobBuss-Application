        ...
        <v-col>
          <div class="d-flex justify-content-between">
            ...
            <div class="pendapatan">
              <div v-for="(count, id) in bookingCounts" :key="id" v-if="item.schedule_id == id">
                <small color="secondary"> Rp.{{ count * item.harga }} </small>
              </div>
              <div v-if="!Object.keys(bookingCounts).includes(String(item.schedule_id))">
                <small color="secondary">Rp.</small>
              </div>
            </div>
...
export default {
  ...
  mounted() {
    ...
    axios.get('/api/schedules/driver', {
      ...
      this.pesanan = response.data.data.filter(pesanan => pesanan.schedules_status == 'complete');
